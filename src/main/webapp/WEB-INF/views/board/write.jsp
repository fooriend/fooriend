...
<head>
	...
	<!--<link rel="stylesheet"href="https://uicdn.toast.com/editor-plugin-chart/latest/toastui-editor-plugin-chart.css"/>

    <link rel="stylesheet"href="https://uicdn.toast.com/editor-plugin-uml/latest/toastui-editor-plugin-uml.css"/>-->
			...
			<div class="form-group">
				<div id="ingredientContainer">
					<div class="ingredient-row">
							<%--@declare id="ingredient1"--%><label for="ingredient1">재료 1</label>
						<input type="text" name="ingredient1" placeholder="재료" required />
						<button class="btn btn-sm btn-primary remove-ingredient" onclick="removeIngredient(this)">취소</button>
					</div>
				</div>
			</div>
			<button class="btn btn-sm btn-primary" onclick="addIngredient()">재료 추가</button>
		</div>

		<script>
			var ingredientCount = 1; // 재료 고유 번호
			...
			function addIngredient() {
				...
				for (var i = 0; i < ingredientRows.length; i++) {
					var label = ingredientRows[i].getElementsByTagName("label")[0];
					label.innerHTML = "재료 " + (i + 1);
					var input = ingredientRows[i].getElementsByTagName("input")[0];
					var currentCount = ingredientCount + (i+1); // 추가된 재료의 번호
					input.name = "ingredient" + currentCount;
				}

				//var currentCount = ingredientCount + 1; // 추가된 재료의 번호
				newIngredientRow.innerHTML =
						...
						currentCount +
						'" placeholder="재료" required />' +
						'<button class="btn btn-sm btn-primary remove-ingredient" onclick="removeIngredient(this)">취소</button>';
				container.appendChild(newIngredientRow);
				//ingredientCount++; // 재료 고유 번호 증가
			}

			function removeIngredient(button) {
				var ingredientRow = button.parentNode;
				ingredientRow.parentNode.removeChild(ingredientRow);

				// 재료 라벨 및 입력 필드 업데이트
				var ingredientRows = document.getElementsByClassName("ingredient-row");
				for (var i = 1; i < ingredientRows.length; i++) {
					var label = ingredientRows[i].getElementsByTagName("label")[0];
					label.innerHTML = "재료 " + (i + 1);
					var input = ingredientRows[i].getElementsByTagName("input")[0];
					var currentCount = ingredientCount + i; // 추가된 재료의 번호
					input.name = "ingredient" + currentCount;
				}
			}
		</script>
				...
				<div class="modal-content">
					...
					<div class="modal-body" style="text-align: center;">
						<a id="thumbnail-tooltip" data-toggle="tooltip" title="썸네일을 등록해주세요😊">
							<img src="../../../resources/img/Fooriend.png" id="thumbnail" width="150px" height="150px">
						</a>
		...
		<script>
			...
			function submit(num){
				...
				if(num==1){
					...
					//form을 submit하기
					$("#insertColumn").attr("action","/board/write").submit();
				}else{
					//임시저장
					$("#insertColumn").children().eq(2).attr("value",$title);
					$("#insertColumn").children().eq(3).attr("value",content);
					$("#insertColumn").attr("action","../save").submit();
				}
			}
		</script>
